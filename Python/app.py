...
def dashboard():
    try:
        # Fetch the latest entry from the raw_data table
        cur.execute("SELECT file_path, time, date, product_id FROM raw_data ORDER BY id DESC LIMIT 1")
        latest_raw_data = cur.fetchone()
        print("Latest raw data:", latest_raw_data)

        # Initialize default values
        file_path, file_time, file_date, product_id = ('None', 'N/A', 'N/A', None)

        if latest_raw_data:
            file_path, file_time, file_date, product_id = latest_raw_data
